...
while [ 1 ]; do
    ...
    xsetroot -name " `echo -e "\Uf17aa"`  $dwm_mem_info `echo -e "\uf063"` $dwm_net_recv_speed `echo -e "\uf062"` $dwm_net_send_speed $dwm_aud_volume $dwm_bright_level [ $dwm_ac_plug,$dwm_bat_power ] $(get_date) "
    sleep $1
done
